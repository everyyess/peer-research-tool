...
function download(apiKey: string): Promise<Buffer> {
...
function checkDartStatus(document: Record<string, unknown>) {
...
async function loadList(apiKey: string): Promise<Map<string, CorpInfo>> {
  const buffer = await download(apiKey);
...
export async function GET(request: NextRequest) {
  ...
  try {
    const corp = (await getList(apiKey)).get(stockCode);
